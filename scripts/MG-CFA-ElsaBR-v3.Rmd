---
title: "Harmonization pilot - Invariance testing for depression and anxiety measures in ELSA Brazil - v3"
...
#imputation
dass_imputed <- mice(dass, m=5, maxit=50, meth='pmm', seed=500)
summary(dass_imputed)
dass_complete <- complete(dass_imputed, 1)
...
mood1 | t1*t1 + t2*t2 + t3*t3
mood2 | t1*t1 + t2*t2 + t3*t3
mood3 | t1*t1 + t2*t2 + t3*t3
mood4 | t1*t1 + t2*t2 + t3*t3

anhe1 | t4*t4 + t5*t5 + t6*t6
anhe2 | t4*t4 + t5*t5 + t6*t6
anhe3 | t4*t4 + t5*t5 + t6*t6
anhe4 | t4*t4 + t5*t5 + t6*t6

guil1 | t7*t7 + t8*t8 + t9*t9
guil2 | t7*t7 + t8*t8 + t9*t9
guil3 | t7*t7 + t8*t8 + t9*t9
guil4 | t7*t7 + t8*t8 + t9*t9

hope1 | t10*t10 + t11*t11 + t12*t12
hope2 | t10*t10 + t11*t11 + t12*t12
hope3 | t10*t10 + t11*t11 + t12*t12
hope4 | t10*t10 + t11*t11 + t12*t12

apat1 | t13*t13 + t14*t14 + t15*t15
apat2 | t13*t13 + t14*t14 + t15*t15
apat3 | t13*t13 + t14*t14 + t15*t15
apat4 | t13*t13 + t14*t14 + t15*t15
'

# Fit
fit_scalar <- cfa(dep_scalar_model, data = dass_complete,
                      ordered = c("mood1", "anhe1", "guil1", "hope1", "apat1",
                                     "mood2", "anhe2", "guil2", "hope2", "apat2",
                                     "mood3", "anhe3", "guil3", "hope3", "apat3",
                                     "mood4", "anhe4", "guil4", "hope4", "apat4"), 
...
str(dass)
colSums(is.na(dass))
dass %>% summarise(across(c(mood1, anhe1, guil1, hope1, apat1, 
                             mood2, anhe2, guil2, hope2, apat2, 
                             mood3, anhe3, guil3, hope3, apat3, 
                             mood4, anhe4, guil4, hope4, apat4), 
                           ~ sum(!is.na(.))))
...
### Results
```{r}
#not shown because did not converge
...
fit_configural <- cfa(dep_configural_model, data = dass,
...
#######james troubleshooting attempt 1#######

#ensure correct number of thresholds
thresholds <- sapply(dass_complete[, c("mood1", "anhe1", "guil1", "hope1", "apat1",
                                       "mood2", "anhe2", "guil2", "hope2", "apat2",
                                       "mood3", "anhe3", "guil3", "hope3", "apat3",
                                       "mood4", "anhe4", "guil4", "hope4", "apat4")], 
...
# define a simpler model to start with
simple_model <- '
dep1 =~ mood1 + anhe1 + guil1 + hope1 + apat1
dep2 =~ mood2 + anhe2 + guil2 + hope2 + apat2
dep3 =~ mood3 + anhe3 + guil3 + hope3 + apat3
...
fit_simple <- cfa(simple_model, data = dass_complete,
                  ordered = c("mood1", "anhe1", "guil1", "hope1", "apat1",
                              "mood2", "anhe2", "guil2", "hope2", "apat2",
                              "mood3", "anhe3", "guil3", "hope3", "apat3",
                              "mood4", "anhe4", "guil4", "hope4", "apat4"), 
...
# check convergence of the simpler model
if (!lavInspect(fit_simple, "converged")) {
  warning("Simple model did not converge. Consider checking data or further simplifying the model.")
} else {
